...
struct SyncAlgorithm {
	SyncAlgorithm(SyncProtocol &sync_protocol, DatabaseClient &client, HashAlgorithm hash_algorithm):
	...
	void check_hash_and_choose_next_range(const Table &table, const ColumnValues *failed_prev_key, const ColumnValues &prev_key, size_t rows_to_hash, const ColumnValues *failed_last_key, size_t target_minimum_block_size, size_t target_maximum_block_size) {
		...
		if (hasher.finish() == hash) {
			...
			if (!failed_last_key) {
				...
				size_t rows_to_failure = client.count_rows(table, hasher.last_key, *failed_last_key);
			...
			hash_failed_range(table, hasher.row_count/2, failed_prev_key, prev_key, hasher.last_key);
	...
	void hash_failed_range(const Table &table, size_t rows_to_hash, const ColumnValues *failed_prev_key, const ColumnValues &prev_key, const ColumnValues &failed_last_key) {
		...
		if (failed_prev_key) {
			sync_protocol.send_rows_and_hash_fail_command(table, *failed_prev_key, prev_key, hasher.row_count, failed_last_key, hasher.finish().to_string());
		} else {
			sync_protocol.send_hash_fail_command(table, prev_key, hasher.row_count, failed_last_key, hasher.finish().to_string());
		}
	}
	...
	void rows_and_next_hash(const Table &table, const ColumnValues &prev_key, ColumnValues last_key, size_t rows_in_range) {
		...
		if (last_key.empty()) {
			...
			if (hasher.row_count == 0) {
				...
				sync_protocol.send_rows_and_hash_next_command(table, prev_key, last_key, hasher.row_count, hasher.finish().to_string());
			}
		}
	}
